...
class ArgoWorkflowsTaskProvider implements TaskProvider {
	...
	List<OptionType> getOptionTypes() {
		...
		OptionType workflowTemplate = new OptionType(
			name: 'Argo Workflows Template',
			code: 'argoWorkflowTemplate',
			fieldName: 'argoWorkflowTemplate',
			displayOrder: 1,
			fieldLabel: 'Template',
			required: true,
			dependsOn: 'argoWorkflowIntegration',
			inputType : OptionType.InputType.SELECT,
			optionSource: 'argoWorkflowTemplates'
		)
		return [nomadJob, workflowTemplate]
	}
